...
Table~\ref{tab:batch-residues} reports the per-residue rate across batch sizes: short sequences gain up to $2.0\times$ at batch~16, medium sequences peak at batch~4 ($1.14\times$), and long sequences are fastest unbatched. The \texttt{--fasta} batcher therefore uses a length-aware schedule by default (a fixed cap is still selectable), which is $1.09$--$1.21\times$ faster than a fixed batch size of 32 on these corpora.
