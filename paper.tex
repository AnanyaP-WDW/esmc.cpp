...
\begin{table}
  \centering
  \caption{Batched corpus embedding on M4 Max Metal F16 (300M). Model loaded once; length-sorted token-budget batches; per-residue rates include one model load and tokenization. Median of 3 interleaved single/batched runs.}
  \label{tab:batching}
  \begin{tabular}{lrrrr}
    \toprule
    Corpus & Tokens/seq & Single (ms/seq) & Batched (ms/seq) & Speedup \\
    \midrule
    short, 2000$\times$45\,aa   & 47        & 10.6  & 5.8   & 1.82$\times$ \\
    medium, 1000$\times$233\,aa & 235       & 32.8  & 33.0  & 0.99$\times$ \\
    long, 100$\times$848\,aa    & 850       & 122.7 & 132.7 & 0.92$\times$ \\
    mixed, 1000 seq             & $\sim$293 & 43.1  & 44.6  & 0.97$\times$ \\
    \bottomrule
  \end{tabular}
\end{table}

Table~\ref{tab:batching} shows that end-to-end the batching benefit is concentrated in short sequences (1.82$\times$), where per-call launch overhead dominates; at medium and long lengths the per-sequence GEMMs already saturate the GPU, and batching is neutral to slightly negative once padding and masked attention are included. Per-residue throughput remains $\sim$6{,}400--7{,}700\,residues/s, versus 1{,}318--2{,}398\,residues/s for PyTorch MPS and 464--1{,}478 for PyTorch CPU, so the runtime itself is the larger win.

\begin{table}
  \centering
  \caption{Per-residue throughput (residues/s) versus batch size on M4 Max Metal F16 over uniform-length sequences; batch 1 is the single-sequence path. Optimal batch size is length-dependent: short favor batch~16, medium batch~4, long batch~1.}
  \label{tab:batch-residues}
  \begin{tabular}{rrrr}
    \toprule
    Batch size & short (45\,aa) & medium (233\,aa) & long (848\,aa) \\
    \midrule
    1  & 4{,}473 & 9{,}253  & \textbf{9{,}719} \\
    2  & 6{,}588 & 10{,}035 & 8{,}531 \\
    4  & 7{,}674 & \textbf{10{,}520} & 8{,}200 \\
    8  & 8{,}385 & 10{,}480 & 7{,}615 \\
    16 & \textbf{8{,}896} & 8{,}922 & 7{,}657 \\
    32 & 8{,}879 & 8{,}682 & 7{,}030 \\
    \bottomrule
  \end{tabular}
\end{table}

Table~\ref{tab:batch-residues} reports the per-residue rate across batch sizes: short sequences gain up to $2.0\times$ at batch~16, medium sequences peak at batch~4 ($1.14\times$), and long sequences are fastest unbatched. The \texttt{--fasta} batcher therefore uses a length-aware schedule by default (a fixed cap is still selectable), which is $1.08$--$1.21\times$ faster than a fixed batch size of 32 on these corpora (Table~\ref{tab:batch-schedule}).

\begin{table}
  \centering
  \caption{Length-aware batch schedule (\texttt{--max-batch 0}) versus a fixed batch size of 32, residues/s on M4 Max Metal F16, median of 3 runs.}
  \label{tab:batch-schedule}
  \begin{tabular}{lrrr}
    \toprule
    Corpus & Auto & Fixed 32 & Gain \\
    \midrule
    short, 2000$\times$45\,aa   & 8{,}030 & 7{,}387 & 1.09$\times$ \\
    medium, 1000$\times$233\,aa & 8{,}855 & 7{,}318 & 1.21$\times$ \\
    long, 100$\times$848\,aa    & 6{,}670 & 6{,}190 & 1.08$\times$ \\
    mixed, 1000 seq             & 7{,}161 & 6{,}380 & 1.12$\times$ \\
    \bottomrule
  \end{tabular}
\end{table}
